...
document.getElementById("updateBtn").addEventListener("click", async () => {
    try {
        ...
        const productId = products.options[products.selectedIndex].value;

        const newStack = document.getElementById("newStack").value;

        const newPrice = document.getElementById("newPrice").value;

        if (newStack === null || newStack === "" || newPrice === null || newPrice === "") throw new Error("You must select the new stack and price");
        ...
        const response = await fetch("https://localhost:9000/v1/Product/Update", config);
